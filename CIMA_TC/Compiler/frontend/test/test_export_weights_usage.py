...
Numpy 格式：
- .npz: converter.export_weights("weights.npz") 或 export_weights(..., format="npz")
  加载: d = np.load("weights.npz"); d.files 为 key 列表（key 中 '.' 已变为 '_'）；d["conv_weight"] 等
- .npy: 仅当单数组时可用；多数组请用 .npz
"""
...
def test_fx_export_weights_load() -> None:
    """FX 路径：导出后能正确 load，且包含 conv/linear 权重 key。"""
    import torch
